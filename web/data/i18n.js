...
'overview.heroSub':      { zh: '跨系统轨迹预测框架', en: 'for 1000+ Biological Systems' },
  ...
  'predict.desc':         { zh: '选择生物系统 → 前96步为输入轨迹 → RegimeFlow 预测引擎生成后256步轨迹预测与置信区间',
                                 en: 'Select system → First 96 steps as input trajectory → RegimeFlow engine generates next 256-step forecast with confidence intervals' },
  'predict.tabModels':    { zh: '模型',           en: 'Models' },
  ...
  'predict.inputHint':    { zh: '前 N 个点 = 输入轨迹（蓝色）· 剩余 = 对比基准（灰色虚线）\n支持 CSV / TSV / 纯数值格式',
                                 en: 'First N points = input trajectory (blue) · Remaining = baseline (gray dashed)\nSupports CSV / TSV / plain numbers' },
  ...
  'predict.subtitleAPI':  { zh: '{sp} · 前96步 (蓝) = 输入轨迹 · 后256步 = 预测区域\n模型: {model} · 推理耗时: {time}ms · 样本数: {samples}',
                                 en: '{sp} · First 96 steps (blue) = Input Trajectory · Next 256 = Prediction\nModel: {model} · Inference: {time}ms · Samples: {samples}' },
  'predict.subtitleOffline': { zh: '{sp} · 前96步 (蓝) = 输入轨迹 · 后256步 = 预测区域\n后端未连接 — 仅显示合成基准数据',
                                 en: '{sp} · First 96 steps (blue) = Input Trajectory · Next 256 = Prediction\nBackend offline — synthetic baseline only' },

  // chart series names
  'chart.context':      { zh: '输入轨迹',                en: 'Input Trajectory' },
